...
int TestsStringCompare(void)
{
	...
	if (LibrariesStringCompare(String_2, String_1))
	{
	...
	if (!LibrariesStringCompare(String_1, String_3))
	{
...
int TestsStringConcatenate(void)
{
	...
	LibrariesStringConcatenate(String_Destination, String_To_Concatenate);
...
int TestsStringConcatenateUpToNumber(void)
{
	...
	// First test
	LibrariesStringConcatenateUpToNumber(String_Destination, String_To_Concatenate, 40);
	// Check the string
	...
	// Second test
	LibrariesStringConcatenateUpToNumber(String_Destination, String_To_Concatenate, 3);
	// Check the string
...
int TestsStringConvertStringToUnsignedInteger(void)
{
	...
	{
		LibrariesScreenWriteString("Testing string \"");
		LibrariesScreenWriteString(String_Numbers[i]);
		LibrariesScreenWriteString("\"...\n");
		
		if (LibrariesStringConvertStringToUnsignedInteger(String_Numbers[i]) != Results[i])
		{
...
int TestsStringConvertUnsignedIntegerToString(void)
{
	...
	{
		LibrariesScreenWriteString("Testing number ");
		LibrariesScreenWriteString(String_Results[i]);
		LibrariesScreenWriteString("...\n");
		
		LibrariesStringConvertUnsignedIntegerToString(Numbers[i], String_Result);
		if (!LibrariesStringCompare(String_Result, String_Results[i]))
		{
...
int TestsStringConvertIntegerToHexadecimal(void)
{
	...
	for (i = 0; i < LIBRARIES_GET_ARRAY_ELEMENTS_COUNT(Numbers); i++)
	{
		...
		LibrariesStringConvertIntegerToHexadecimal((int) Numbers[i], String_Result);
		if (!LibrariesStringCompare(String_Result, String_Results[i]))
		{
...
int TestsStringCopy(void)
{
	...
	LibrariesStringCopy(String_Source, String_Destination);
...
int TestsStringGetSize(void)
{
	...
	if (LibrariesStringGetSize(String_Empty) != 0)
	{
	...
	if (LibrariesStringGetSize(String) != sizeof(String) - 1) // -1 to remove the terminating zero automatically added by the compiler
	{
