...
TFileSystemMasterBootLoaderPartitionTableEntry *ShellInstallerPartitionMenu(void)
{
	...
	ShellInstallerDisplayTitle(STRING_SHELL_INSTALLER_PARTITION_MENU_SHOW_PARTITION_TABLE_1);
	ScreenWriteString(STRING_SHELL_INSTALLER_PARTITION_MENU_SHOW_PARTITION_TABLE_2);
	...
	{
		// Partition number
		ScreenWriteString(STRING_SHELL_INSTALLER_PARTITION_MENU_PARTITIONS_DETAILS_1);
		ScreenWriteString(itoa(i + 1)); // Partitions start from 1
		ScreenWriteString(STRING_SHELL_INSTALLER_PARTITION_MENU_PARTITIONS_DETAILS_2);
		ScreenWriteString(itoa(Partition_Table[i].Type));
		if (Partition_Table[i].Type != 0)
		{
			ScreenWriteString(STRING_SHELL_INSTALLER_PARTITION_MENU_PARTITIONS_DETAILS_3);
			ScreenWriteString(itoa(Partition_Table[i].First_Sector_LBA));
			ScreenWriteString(STRING_SHELL_INSTALLER_PARTITION_MENU_PARTITIONS_DETAILS_4);
			ScreenWriteString(itoa(Partition_Table[i].Sectors_Count * FILE_SYSTEM_SECTOR_SIZE_BYTES));
		}
		else ScreenWriteString(STRING_SHELL_INSTALLER_PARTITION_MENU_PARTITIONS_DETAILS_EMPTY_PARTITION);
	...
	do
	{
		ScreenWriteString(STRING_SHELL_INSTALLER_PARTITION_MENU_CHOOSE_PARTITION_NUMBER);
